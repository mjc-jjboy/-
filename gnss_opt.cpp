...
// ------------------------ Experiments ------------------------
struct Options {
    string exp="ls"; // ls / kf_static / kf_dyn_cv / kf_dyn_ca
    string obsPath;
    string navPath;
...
static void printUsage(){
    cerr << "Usage: gnss_opt --exp <ls|kf_static|kf_dyn_cv|kf_dyn_ca> --obs <obs.rnx> --nav <nav.rnx>\n"
         << "                [--sys <A|G|C>] [--max-epochs N] [--out report.txt]\n";
}
...
int main(int argc, char**argv){
    ...
    if(opt.exp=="ls"){
        ok = runExpLS(opt, epochs, info, nav, initPos);
    }else if(opt.exp=="kf_static"){
        ok = runExpKFStatic(opt, epochs, info, nav, initPos);
    }else if(opt.exp=="kf_dyn_cv"){
        ok = runExpKFDynamic(opt, epochs, info, nav, false, initPos);
    }else if(opt.exp=="kf_dyn_ca"){
        ok = runExpKFDynamic(opt, epochs, info, nav, true, initPos);
    }else{
        cerr<<"Unknown experiment: "<<opt.exp<<"\n";
        printUsage();
        return 1;
    }
